...
export const UserSignup: React.FC<UserSignupProps> = ({
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      setErrorMessage(err?.message || 'An unexpected error occurred during Google signup.');
      setGoogleLoading(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            <div>
              <input
                ...
                className={`w-full px-4 py-3 border text-[11px] sm:text-xs lg:text-sm rounded-md focus:outline-none transition-colors ${errors.firstName
